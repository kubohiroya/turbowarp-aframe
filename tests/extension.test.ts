...
describe('TurboWarpAFrameExtension', () => {
  ...
  it('exports and imports scene graphs as YAML DSL', () => {
    const extension = new TurboWarpAFrameExtension();
    extension.createScene({LAYER: 'above-stage', MODE: '3d'});
    extension.createNode({TYPE: 'box', ID: 'card', PARENT: '#scene'});
    extension.addClass({CLASS: 'monster', SELECTOR: '#card'});
    extension.setData({SELECTOR: '#card', KEY: 'zone', VALUE: 'field'});
    extension.setAttribute({SELECTOR: '#card', NAME: 'material', VALUE: 'color: #4cffb0'});
    extension.setPosition({SELECTOR: '#card', X: 0, Y: 1, Z: -3});

    const yaml = extension.sceneYaml();
    expect(yaml).toContain('formatVersion: 1');
    expect(yaml).toContain('id: card');
    expect(yaml).toContain('zone: field');

    const restored = new TurboWarpAFrameExtension();
    restored.loadSceneYaml({SOURCE: yaml});

    expect(restored.countSelector({SELECTOR: '.monster'})).toBe(1);
    expect(restored.countSelector({SELECTOR: '[data-zone=field]'})).toBe(1);
    restored.moveBy({SELECTOR: '#card', X: 1, Y: 0, Z: 0});
    expect(restored.snapshot()).toMatchObject({
      options: {layer: 'above-stage', mode: '3d'},
      nodes: expect.arrayContaining([
        expect.objectContaining({
          id: 'card',
          attributes: expect.objectContaining({position: '1 1 -3'})
        })
      ])
    });
  });

  it('loads handwritten YAML DSL scenes', () => {
    const extension = new TurboWarpAFrameExtension();
    extension.loadSceneYaml({
      SOURCE: [
        'formatVersion: 1',
        'options:',
    ...
    expect(extension.whenSceneReady()).toBe(true);
    expect(extension.countSelector({SELECTOR: '#monster-card'})).toBe(1);
    expect(extension.countSelector({SELECTOR: '.monster'})).toBe(1);
    expect(extension.countSelector({SELECTOR: '[data-zone=field]'})).toBe(1);
    expect(extension.snapshot()).toMatchObject({
      options: {layer: 'camera-under-3d', mode: 'ar-fallback'}
    });
  });

  it('generates deterministic fallback ids for YAML children without ids', () => {
    const extension = new TurboWarpAFrameExtension();
    extension.loadSceneYaml({
      SOURCE: [
        'formatVersion: 1',
    ...
    expect(extension.countSelector({SELECTOR: '.card'})).toBe(2);
    expect(extension.countSelector({SELECTOR: '#scene-box-1'})).toBe(1);
    expect(extension.countSelector({SELECTOR: '#scene-box-2'})).toBe(1);
  });

  it('rejects malformed YAML DSL scenes', () => {
    const extension = new TurboWarpAFrameExtension();

    expect(() => extension.loadSceneYaml({SOURCE: 'formatVersion: 1\nextra: true\nroot: {}\n'}))
      .toThrow('A-Frame scene YAML extra is not supported.');
    expect(() =>
      extension.loadSceneYaml({SOURCE: 'formatVersion: 1\noptions:\n  extra: true\nroot: {}\n'})
    ).toThrow('A-Frame scene YAML options.extra is not supported.');
    expect(() => extension.loadSceneYaml({SOURCE: 'formatVersion: 2\nroot: {}\n'})).toThrow(
      'A-Frame scene YAML formatVersion must be 1.'
    );
    expect(() => extension.loadSceneYaml({SOURCE: 'formatVersion: 1\nroot:\n  children: nope\n'}))
      .toThrow('A-Frame scene.root.children must be an array.');
  });

  it('keeps the existing graph when YAML import fails preflight', () => {
    const extension = new TurboWarpAFrameExtension();
    extension.createScene({LAYER: 'above-stage', MODE: '3d'});
    extension.createNode({TYPE: 'box', ID: 'existing', PARENT: '#scene'});
    ...
    ).toThrow('Duplicate A-Frame node id in scene YAML: duplicate');
    expect(extension.countSelector({SELECTOR: '#existing'})).toBe(1);
    expect(extension.countSelector({SELECTOR: '#duplicate'})).toBe(0);
  });
});
