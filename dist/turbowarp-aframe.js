...
(function (Scratch) {
  ...
  const SCALAR$1 = /* @__PURE__ */ Symbol.for("yaml.scalar");
  ...
  const SCENE_YAML_KEYS = /* @__PURE__ */ new Set(["formatVersion", "options", "root"]);
  const SCENE_OPTION_KEYS = /* @__PURE__ */ new Set(["layer", "mode"]);
  class TurboWarpAFrameExtension {
    ...
    loadSceneYaml(args) {
      const document2 = this.parseSceneYaml(Scratch.Cast.toString(args.SOURCE));
      this.preflightSceneYaml(document2);
      this.sceneOptions = {
        layer: document2.options?.layer ?? "above-stage",
        mode: document2.options?.mode ?? "3d"
      };
      this.resetGraph();
      this.rootElement = this.createSceneElement();
      const root = this.requireNode(ROOT_ID);
      root.element = this.rootElement;
      this.applyTemplateToRoot(root, document2.root);
      this.applyNodeToElement(root);
      for (const [index, child] of (document2.root.children ?? []).entries()) {
    ...
    addSceneNodeTree(template, parentId, fallbackId) {
    ...
    parseSceneYaml(source) {
      const value = parse(source);
      this.validateSceneYamlDocument(value);
      return value;
    }
    validateSceneYamlDocument(value) {
      ...
      if (document2["formatVersion"] !== 1) {
        throw new TypeError("A-Frame scene YAML formatVersion must be 1.");
      }
      if (document2["options"] !== void 0) {
        if (typeof document2["options"] !== "object" || document2["options"] === null || Array.isArray(document2["options"])) {
          throw new TypeError("A-Frame scene YAML options must be an object.");
        }
        const options = document2["options"];
      ...
      this.validateTemplateNode(document2["root"], "scene.root");
    }
    preflightSceneYaml(document2) {
      const ids = /* @__PURE__ */ new Set([ROOT_ID]);
      for (const [index, child] of (document2.root.children ?? []).entries()) {
        this.collectSceneNodeIds(child, ROOT_ID, index, ids);
      }
    }
    collectSceneNodeIds(template, parentId, index, ids) {
    ...
    sceneNodeFallbackId(template, parentId, index) {
      const type = this.normalizeId(template.type ?? "node");
      return `${parentId}-${type}-${index + 1}`;
    }
    applyTemplateToRoot(root, template) {
