...
function checkPolicy() {
  ...
  if (policy.productName !== 'TurboWarp-A-Frame') {
...
function checkPackageMetadata() {
  ...
  if (packageMetadata.repository?.url !== 'git+https://github.com/kubohiroya/turbowarp-aframe.git') {
    errors.push('package.json repository.url must point to the current repository');
  }
  if (packageMetadata.bugs?.url !== 'https://github.com/kubohiroya/turbowarp-aframe/issues') {
...
function checkReadmes() {
  ...
  if (!readmeJa.startsWith('# TurboWarp-A-Frame\n')) {
...
async function checkPackContents() {
  ...
  if (!files.has('dist/turbowarp-aframe.js')) {
